...
function renderInbox(inbox) {
  const rows = inbox.findings
    .slice(0, 8)
    .map(
      (finding) => `<tr>
        <td><a href="/?${queryString({ session: finding.session, severity: "high", status: "open" })}">${escapeHtml(finding.session)}</a></td>
  ...
  return `<section class="inbox" aria-label="Review inbox">
    <div class="inbox-header">
      <div>
        <p class="eyebrow">Review Inbox</p>
        <h2>${inbox.count} open high findings</h2>
      </div>
      <span>${inbox.sessions} sessions scanned</span>
    </div>
    ${
      inbox.findings.length
        ? `<table>
            <thead><tr><th>Session</th><th>Status</th><th>Agent</th><th>Location</th><th>Summary</th></tr></thead>
            <tbody>${rows}</tbody>
          </table>`
